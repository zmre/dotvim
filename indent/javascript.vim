...
function! GetJsIndent()
    ...
    let ind = indent(pnum)
		...
		" add extra indent when three open brackets in a row
		if pline =~ '[\[{(]\s*[\[{(]\s*[\[{(]\s*$'
			let ind = ind + &sw
    endif

		" add extra indent if ( followed by optional () followed by open bracket
		if pline =~ '([^)]*\(([^)]*)\)*[\[{(]\s*$\|[\[{(]\s*[\[{(]\s*$'
			let ind = ind + &sw
    endif
    ...
    if pline =~ '{\s*$\|[\s*$\|(\s*$'
		...
		" if cur line is two close brackets, unindent extra
		if line =~ '^\s*[}\])]\s*[}\])]\s*[;,]*\s*[;,]*\s*$'
				let ind = ind - &sw
		endif
